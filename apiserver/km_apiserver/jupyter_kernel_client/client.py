...
class JupyterKernelClient:
    ...
    async def acreate(
            ...
            "spec": {
                ...
                "template": {
                    "spec": {
                        "containers": [
                            {
                                "env": [{"name": k, "value": v} for k, v in payload.model_dump(by_alias=True).items()],
        ...
        try:
            ...
            if e.status == HTTPStatus.FORBIDDEN.value and "exceeded quota:" in e.body:
        ...
        if wait_for_ready:
            ...
            except (KernelWaitReadyTimeoutError, KernelRetrieveError) as e:
                error_msg = (
                    f"Kernel {payload.kernel_id} in namespace {payload.kernel_namespace} waiting too long. deleting it."
                )
                self.logger.exception(error_msg)
                await self.adelete_by_kernel_id(kernel_id=payload.kernel_id, namespace=payload.kernel_namespace)
                raise KernelCreationError(error_msg) from e
        ...
        while True:
            try:
                ...
                if time.time() - start_time > timeout:
                    ...
                    raise KernelWaitReadyTimeoutError(error_msg)  # noqa: TRY301
